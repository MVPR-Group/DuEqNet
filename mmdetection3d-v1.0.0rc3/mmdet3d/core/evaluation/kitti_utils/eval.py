...
def clean_data(gt_anno, dt_anno, current_class, difficulty):
    ...
    num_valid_gt = 0
    for i in range(num_gt):  # 每一个gt box
        bbox = gt_anno['bbox'][i]
        gt_name = gt_anno['name'][i].lower()  # 将类名字转成全部小写
        height = bbox[3] - bbox[1]
        valid_class = -1
        if (gt_name == current_cls_name):  # 类别判断
            valid_class = 1
    ...
    # for i in range(num_gt):
        if gt_anno['name'][i] == 'DontCare':
            dc_bboxes.append(gt_anno['bbox'][i])  # dontcare类box
    for i in range(num_dt):  # 每一个dt box
...
def calculate_iou_partly(gt_annos, dt_annos, metric, num_parts=50):
    ...
    total_dt_num = np.stack([len(a['name']) for a in dt_annos], 0)  # 每个sample预测得到的box数量
    total_gt_num = np.stack([len(a['name']) for a in gt_annos], 0)  # 每个sample的gtboxe数量
    num_examples = len(gt_annos)
    split_parts = get_split_parts(num_examples, num_parts)
    parted_overlaps = []
    example_idx = 0

    for num_part in split_parts:  # 分割成一小部分一小部分来计算IOU
        gt_annos_part = gt_annos[example_idx:example_idx + num_part]
        dt_annos_part = dt_annos[example_idx:example_idx + num_part]
        if metric == 0:  # 2d iou计算
            gt_boxes = np.concatenate([a['bbox'] for a in gt_annos_part], 0)  # 取gt_sample里的gt_boxes，一个sample里有若干个boxes
            dt_boxes = np.concatenate([a['bbox'] for a in dt_annos_part], 0)
            overlap_part = image_box_overlap(gt_boxes, dt_boxes)
        elif metric == 1:  # bev下的ious计算
            loc = np.concatenate(
    ...
    for j, num_part in enumerate(split_parts):
        ...
        for i in range(num_part):
            ...
            # parted_overlaps是若干个sample的gt和dt box一起算的iou
            # overlaps则要将若干个sample再拆分成每一个sample的gt和dt box的iou
...
def _prepare_data(gt_annos, dt_annos, current_class, difficulty):
    ...
    for i in range(len(gt_annos)):
        ...
        # num_valid_gt 表示有效的gt box的数量。 ignored_gt 表示是否需要忽略的标记，1表示需要忽略
        # ignored_det 表示是否需要忽略的标记，1表示要忽略。 dc_bboxes 表示dontcare类的box
        ...
        gt_datas = np.concatenate(
            [gt_annos[i]['bbox'], gt_annos[i]['alpha'][..., np.newaxis]], 1)  # 将角度也拼在一起
...
def eval_class(gt_annos,
    ...
    rets = calculate_iou_partly(dt_annos, gt_annos, metric, num_parts)  # 在不同的视角下计算IOU
    overlaps, parted_overlaps, total_dt_num, total_gt_num = rets
    # overlaps是每一个sample，gt和dt的iou情况。parted_overlaps则是若干个sample拼在一起的gt和dt的iou情况。
    # total_dt_num和total_gt_num则是每一个sample得到的预测box和gt box的实际数量
    N_SAMPLE_PTS = 41
    ...
    for m, current_class in enumerate(current_classes):  # 每一类
        for idx_l, difficulty in enumerate(difficultys):  # 每一难度
            rets = _prepare_data(gt_annos, dt_annos, current_class, difficulty)  # 根据难度、类别对gt和dt box进行筛选
            (gt_datas_list, dt_datas_list, ignored_gts, ignored_dets,
             dontcares, total_dc_num, total_num_valid_gt) = rets
            # gt_data_list (list of array(x, 5) )表示 N个samples，每个sample的若干gt box
            # dt_data_list 表示 N个samples，每个sample的若干dt box
            # ignored_gts 标记每一个sample的每一个gt box是否有效，有效为1。 ignored_dets 同 ignored_gts
            # dontcares 表示 N个samples，每一个sample里的 dontcare类box
            # total_dc_num 表示每一个sample里dontcare类的数量。total_num_valid_gt表示所有的有效gt的数量

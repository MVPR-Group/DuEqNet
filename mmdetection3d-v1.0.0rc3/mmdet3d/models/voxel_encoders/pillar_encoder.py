...
class PillarFeatureNet(nn.Module):
    ...
    def forward(self, features, num_points, coors):
        ...
        # if self._with_voxel_center:
        #     if not self.legacy:
        #         f_center = torch.zeros_like(point_coords[:, :, :2])
        #         f_center[:, :, 0] = point_coords[:, :, 0] - (
        #                 coors[:, 3].to(dtype).unsqueeze(1) * self.vx +
        #                 self.x_offset)
        #         f_center[:, :, 1] = point_coords[:, :, 1] - (
        #                 coors[:, 2].to(dtype).unsqueeze(1) * self.vy +
        #                 self.y_offset)
        #     else:
        #         f_center = point_coords[:, :, :2]  # xy
        #         f_center[:, :, 0] = f_center[:, :, 0] - (
        #                 coors[:, 3].type_as(point_coords).unsqueeze(1) * self.vx +
        #                 self.x_offset)  # x
        #         f_center[:, :, 1] = f_center[:, :, 1] - (
        #                 coors[:, 2].type_as(point_coords).unsqueeze(1) * self.vy +
        #                 self.y_offset)  # y
        #     features_ls.append(f_center)  # c += 2

        if self._with_distance:
            points_dist = torch.norm(point_coords, 2, 2, keepdim=True)
